...
execute as @a store result score @s contracttimer run resource get @s engineers:contracting_timer
execute as @a[tag=contractor] unless score @s contracttimer matches 0 run function engineers:contractor/repeat
execute as @a at @s if score @s trades matches 25.. unless score @s contractorswitch matches 1 run function engineers:contractor/achievement
execute as @a at @s if score @s trades matches 25.. if entity @e[type=item,limit=1,sort=nearest,nbt={Item: {id: "minecraft:paper", Count:1b}}] at @e[type=item,limit=1,sort=nearest,nbt={Item: {id: "minecraft:paper", Count:1b}}] if block ~ ~-0.1 ~ emerald_block run function engineers:contractor/paper

# End
execute as @a at @s if score @s trades matches 25.. if entity @e[type=item,limit=1,sort=nearest,nbt={Item: {id: "minecraft:paper", Count:1b}}] at @e[type=item,limit=1,sort=nearest,nbt={Item: {id: "minecraft:paper", Count:1b}}] unless block ~ ~-0.1 ~ emerald_block run scoreboard players set @s timer 0
